...
public class JdbcReservationRepository implements ReservationRepository {
    ...
    public Optional<Reservation> findById(Long id) {
        String sql = "SELECT "
                ...
                + "WHERE r.id = :id AND r.status='ACTIVE' AND r.deleted_at = 0 ";
    ...
    public List<Reservation> findAll() {
        String sql = "SELECT "
                ...
                + "INNER JOIN theme t ON r.theme_id = t.id "
                + "INNER JOIN reservation_time rt ON r.time_id = rt.id "
                + "WHERE r.status = 'ACTIVE' AND r.deleted_at = 0 "
    ...
    public List<Reservation> findByThemeAndDate(Long themeId, LocalDate date) {
        String sql = "SELECT "
                ...
                + "INNER JOIN theme t ON r.theme_id = t.id "
                + "INNER JOIN reservation_time rt ON r.time_id = rt.id "
                + "WHERE r.theme_id = :themeId AND r.date = :date AND r.status = 'ACTIVE' AND r.deleted_at = 0";
    ...
    public List<Reservation> findAllByName(String username) {
        String sql = "SELECT "
                ...
                + "INNER JOIN theme t ON r.theme_id = t.id "
                + "INNER JOIN reservation_time rt ON r.time_id = rt.id "
                + "WHERE r.name = :username AND r.status = 'ACTIVE' AND r.deleted_at = 0";
        return jdbcTemplate.query(sql, Map.of("username", username), rowMapper);
    }

    @Override
    public boolean existsByReservationTime(Long timeId) {
        String sql = "SELECT EXISTS (SELECT 1 FROM reservation WHERE time_id=:timeId AND status='ACTIVE' AND deleted_at = 0)";
    ...
    public boolean existsByReservationTimeAndThemeAndDate(Long timeId, Long themeId, LocalDate date) {
        String sql = "SELECT EXISTS (SELECT 1 FROM reservation WHERE time_id=:timeId AND theme_id=:themeId AND date=:date AND status='ACTIVE' AND deleted_at = 0)";
    ...
    public boolean existsByIdAndUsernameAndActive(Long reservationId, String username) {
        String sql = "SELECT EXISTS (SELECT 1 FROM reservation WHERE id=:reservationId AND name=:username AND status='ACTIVE' AND deleted_at = 0)";
    ...
    public boolean existsByReservationTimeAndThemeAndDateAndIdNot(Long id, Long timeId, Long themeId, LocalDate date) {
        String sql = "SELECT EXISTS (SELECT 1 FROM reservation WHERE id != :id AND time_id=:timeId AND theme_id=:themeId AND date=:date AND status='ACTIVE' AND deleted_at = 0)";
    ...
    public boolean existsByTheme(Long themeId) {
        String sql = "SELECT EXISTS (SELECT 1 FROM reservation WHERE theme_id=:themeId AND deleted_at = 0)";
    ...
    public void cancel(Reservation reservation) {
        String sql = "UPDATE reservation SET status = 'CANCELED', deleted_at=:id WHERE id = :id AND status='ACTIVE'";
